...
# Generate the X-Gorgon and X-Khronos header to send API requests
def getcryptLib(param="", stub="", cookie=""):
    gorgon = []
    ttime = time()
    ...
    return {"X-Gorgon": cryptLib(gorgon).main(), "X-Khronos": str(int(ttime))}
...
# return the X-Gorgon and X-Khronos headers
def getcryptLib_m(param, data):
    return getcryptLib(param, hashlib.md5(data.encode()).hexdigest() if data else None, None)

def get_profile(session_id, device_id, UUID):
    """Retrieve the current TikTok username for a given session, device, and UUID."""
    try:
        data = None
        parm = (f"device_id={device_id}&UUID={UUID}&id=kaa&version_code=34.0.0&language=en"
...
def get_profile_us(session_id, device_id, UUID):
    try:
        data = None
        parm = (f"device_id={device_id}&UUID={UUID}&id=kaa&version_code=34.0.0&language=en"
                "&app_name=lite&app_version=34.0.0&carrier_region=SA&tz_offset=10800&mcc_mnc=42001"
...
def check_is_changed(last_username, session_id, device_id, UUID):
    return get_profile(session_id, device_id, UUID) != last_username

def check_is_changed_us(last_username, session_id, device_id, UUID):
    return get_profile_us(session_id, device_id, UUID) != last_username

# EU/Asia/SA endpoint
def change_username(session_id, device_id, UUID, last_username, new_username):
    """Attempt to change a TikTok username."""
    data = f"unique_id={quote(new_username)}"
    parm = (f"device_id={device_id}&UUID={UUID}&residence=SA&version_name=1.1.0&os_version=17.4.1"
            "&app_name=tiktok_snail&locale=en&ac=4G&sys_region=SA&version_code=1.1.0&channel=App%20Store"
            "&op_region=SA&os_api=18&device_brand=iphone&idfv={UUID}-1ED5-4350-9318-77A1469C0B89"
            ...
            "&cdid={UUID}&openudid={UUID}&app_version=1.1.0&scene_id=830")
    sig = getcryptLib_m(parm, data)
    ...
    headers = {
        ...
        "Cookie": f"sessionid={session_id}",
        "sdk-version": "2",
        "user-agent": "com.zhiliaoapp.musically/{device_id} (Linux; U; Android 5; en; {UUID}; Build/PI;tt-ok/3.12.13.1)",
        "X-Gorgon": sig["X-Gorgon"],
        "X-Khronos": sig["X-Khronos"],
    }
    response = requests.post(url, data=data, headers=headers)
    result = response.text
    if "unique_id" in result and check_is_changed(last_username, session_id, device_id, UUID):
        return "Username change successful."
    else:
        return "Failed to change username: " + str(result)

# US and EU/Asia/SA versions of TikTok have different API endpoints for changing usernames
def change_username_us(session_id, device_id, UUID, last_username, new_username):
    ...
    response = requests.post(url, data=data, headers=headers)
    result = response.text
    if "unique_id" in result and check_is_changed_us(last_username, session_id, device_id, UUID):
        return "Username change successful."
    else:
        return "Failed to change username: " + str(result)
...
def main():
    ...
    UUID = str(random.randint(777777788, 999999999999))
    session_id = input("Enter the sessionid: ")

    user = get_profile(session_id, device_id, UUID)
    if user != "None":
        print(f"Your current TikTok username is: {user}")
        new_username = input("Enter the new username you wish to set: ")
        print(change_username(session_id, device_id, UUID, user, new_username))
    else:
        user_us = get_profile_us(session_id, device_id, UUID)
        print(user_us)
        if user_us != "None":
            print(f"Your current TikTok username is: {user_us}")
            new_username = input("Enter the new username you wish to set: ")
            print(change_username_us(session_id, device_id, UUID, user_us, new_username))
        else:
